...
CREATE TABLE `inventory` (
  ...
  `kode_barang` char(16) NOT NULL,
  `serial_number` char(16) NOT NULL,
  `no_invoice` char(16) NOT NULL,
  `nama_barang` varchar(255) NOT NULL,
  ...
  `jenis_id` int(11) NOT NULL,
  `gudang_id` int(11) NOT NULL,
  `pemeriksa` varchar(225) DEFAULT NULL,
  `tanggal_pemeriksaan` date NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

INSERT INTO `inventory` (`id_inventory`, `kode_barang`, `serial_number`, `no_invoice`, `nama_barang`, `jumlah`, `satuan_id`, `kondisi_id`, `status_id`, `jenis_id`, `gudang_id`, `pemeriksa`, `tanggal_pemeriksaan`) VALUES (1, '001', '12345', 'lt.', 'Monitor LED Full HD', 1, 1, 1, 1, 3, 2, 'John Doe', '2023-01-28');

ALTER TABLE `inventory`
  ADD PRIMARY KEY (`id_inventory`),
  ADD KEY `satuan_id` (`satuan_id`),
  ADD KEY `kondisi_id` (`kondisi_id`),
  ADD KEY `status_id` (`status_id`),
  ADD KEY `jenis_id` (`jenis_id`),
  ADD KEY `gudang_id` (`gudang_id`);

ALTER TABLE `inventory`
  ADD CONSTRAINT `inventory_ibfk_1` FOREIGN KEY (`satuan_id`) REFERENCES `satuan` (`id_satuan`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `inventory_ibfk_2` FOREIGN KEY (`kondisi_id`) REFERENCES `kondisi` (`id_kondisi`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `inventory_ibfk_3` FOREIGN KEY (`status_id`) REFERENCES `status` (`id_status`) ON DELETE CASCADE ON UPDATE CASCADE;
