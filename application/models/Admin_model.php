...
class Admin_model extends CI_Model
{
	...
	public function getInventory()
	{
        ...
        $this->db->join('kondisi k', 'b.kondisi_id = k.id_kondisi');
        $this->db->join('status t', 'b.status_id = t.id_status');
		$this->db->order_by('id_inventory');
		return $this->db->get('inventory b')->result_array();
	}
	...
	public function gudangAvailable()
	{
		$this->db->distinct();
		$this->db->select('g.id_gudang, g.nama_gudang');
		$this->db->join('inventory i', 'g.id_gudang = i.gudang_id');
		$this->db->where('g.id_gudang = i.gudang_id');
		return $this->db->get('gudang g')->result_array();
	}

	public function chartPie($g, $s)
	{
		$this->db->select('t.nama_status, g.nama_gudang');
		$this->db->join('status t', 'i.status_id = t.id_status');
		$this->db->join('gudang g', 'i.gudang_id = g.id_gudang');
		$this->db->where('i.gudang_id' . ' =', $g['id_gudang']);
		$this->db->where('i.status_id' . ' =', $s);
		return count($this->db->get('inventory i')->result_array());
	}
}
