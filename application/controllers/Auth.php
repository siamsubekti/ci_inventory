...
class Auth extends CI_Controller
{
    ...
    public function register()
    {
        ...
        $this->form_validation->set_rules('password', 'Password', 'trim|required|min_length[8]|xss_clean|callback_is_password_strong');
        $this->form_validation->set_rules('password2', 'Konfirmasi Password', 'matches[password]|trim');
        $this->form_validation->set_rules('nama', 'Nama', 'required|trim');
        $this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email|is_unique[user.email]');
        $this->form_validation->set_rules('no_telp', 'Nomor Telepon', 'required|trim');
		$this->form_validation->set_rules('NIP', 'NIP', 'required|trim');
		$this->form_validation->set_message('is_password_strong','password harus mengandung capital case, karakter spesial, dan minimal 8 karakter');
